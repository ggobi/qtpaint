...
interface to the \Qt{} library.  Most of the time you will need to load both \Rpackage{qtbase} and \Rpackage{qtpaint} packages.
...
\Rclass{QTransform} is performed by the \Rfunction{qvmap} function. The
